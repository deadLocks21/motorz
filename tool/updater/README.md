...
Installateur / launcher / **auto-updater silencieux** pour Motorz desktop
...
release GitHub (`motorz-updater-windows.exe`, `motorz-updater-linux`).
...
La fenêtre est **rendue par l'app Motorz elle-même** (`current/motorz
...
<root>/                      racine choisie à l'install
  versions/<v>/              contenu d'une version (motorz.exe + dll + data/, ou AppImage)
  current      ->  versions/<v>   jonction (Windows) / symlink (Linux)
  updater/motorz-updater[.exe]   le binaire relocalisé (cible stable des raccourcis)
...
Racine par défaut : `%LOCALAPPDATA%\Motorz` (Windows),
`~/.local/share/Motorz` (Linux).
...
L'updater est un binaire **distinct** de `motorz.exe`, et il met à jour
...
cd tool/updater
dart pub get
dart compile exe bin/motorz_updater.dart -o motorz-updater
```
