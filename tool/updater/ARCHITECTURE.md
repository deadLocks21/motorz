...
Conçu pour Motorz mais pensé pour être **répliqué** : la fin du document liste
...
```
<root>/                         %LOCALAPPDATA%\Motorz  (Win) | ~/.local/share/Motorz (Linux)
  versions/
    1.10.1/                     contenu d'une version (exe + dll + data/, ou AppImage)
    1.10.2/
  current        ->  versions/1.10.2     jonction (Windows) / symlink (Linux)
  updater/
    motorz-updater(.exe)       le binaire updater, relocalisé ici (cible des raccourcis)
...
`updater/motorz-updater`, **jamais** sur un exe versionné → il survit à toutes
...
- Assets attendus par l'updater (à matcher par regex / nom) :
  - App Windows : `motorz-windows-<v>-<run>.zip` (contenu du dossier `Release/`).
  - App Linux : `Motorz-<v>-<run>-x86_64.AppImage`.
  - Updater : `motorz-updater-windows.exe`, `motorz-updater-linux`.
...
- `bin/motorz_updater.dart` — entrée.
...
2. **Adapter `lib/layout.dart`** :
   ...
   - `defaultRoot()` / `pointerFile()` : remplacer « Motorz » par le nom de l'app.
