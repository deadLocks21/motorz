...
/// Décrit l'arborescence d'une installation Motorz enracinée en [root] :
///
/// ```
/// <root>/
...
class Layout {
  ...
  static String pointerFile() {
    if (Platform.isWindows) {
      ...
      return p.join(appData, 'Motorz', 'install.path');
    }
    final xdg = _env('XDG_CONFIG_HOME');
    final base = (xdg != null && xdg.isNotEmpty)
        ? xdg
        : p.join(_env('HOME') ?? '.', '.config');
    return p.join(base, 'motorz', 'install.path');
  }
