...
Future<File> downloadAsset(ReleaseAsset asset, String tmpDir, Log log) async {
  ...
  httpDownload(
    asset.url,
    dest.path,
    headers: {'User-Agent': 'motorz-updater'},
  );
...
///  - Windows : décompresse le `.zip` (motorz.exe + dll + data/) à la racine.
///  - Linux   : place l'AppImage en `Motorz.AppImage` et la rend exécutable.
void installAppArtifact(File archiveFile, String versionDir, Log log) {
  ...
  if (Platform.isWindows) {
    ...
    final target = File(p.join(versionDir, 'Motorz.AppImage'));
