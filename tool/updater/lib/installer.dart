...
class Installer {
  ...
  ///
  /// Motorz n'a pas de base desktop pré-existante : cette première release
  /// desktop embarque déjà `--updating`. Garder cette valeur <= à la version
  /// de la 1ʳᵉ release desktop publiée (semantic-release : un `feat` part de
  /// 1.4.0 -> 1.5.0).
  ...
  /// Installe Motorz dans la racine de [layout]. [launch] démarre l'app à la
  ...
  Future<void> install({bool launch = true}) async {
    ...
    log('Installation terminée : Motorz ${release.version}');
    if (launch) launchApp();
  }
